...
**Documentation date:** 2026-09-18, round 7 — latest: live cart-aware stock display on the product grid (adding an item to cart visually deducts from its displayed stock immediately, client-side only — the real Inventory number is never touched until checkout actually succeeds). Everything shipped across all rounds today is listed in §0's "Finished and verified" bullets below (per-field account validation + live current-password check, a page-load performance pass, plain-English audit log, email password reset, receipt/barcode/CSV/audit-log features, and more) — read §0 rather than this header for the full list; the rest of this file is otherwise unchanged and still accurate.
...
- **Per-field validation + live current-password checking on My Account** — the Change Password and Change Email forms previously showed one generic error banner for the whole form (e.g. "Your current password is incorrect." floating above the button, not tied to any field) — worse, a client-side "passwords don't match" check ran *before* the server-side current-password check, so a cashier with both a wrong current password and a mismatched confirmation would only ever see the mismatch error and never find out their current password was also wrong until they fixed that first. Now: every field gets its own inline red error under itself (matching the login page's pattern), all client-side checks run together so every mistake shows at once, and a wrong current password is pointed at specifically. Also added **live checking**: a new `POST /account/verify-current-password` endpoint (rate-limited, `throttle:20,1`) is called in the background when the Current Password field loses focus, so a cashier finds out their password is wrong before they've even finished filling out the rest of the form — the same live-feedback feel as the login page's email check, but necessarily a real server round-trip (not a pure browser check) since only the server has the actual password hash to compare against.
- **Page-load performance pass** — `bootstrap()` (main POS page), `managerBootstrap()` (Manager Console), `refreshReports()`, `loadProductAnalytics()`, and the post-checkout refresh were all firing 3-6 independent API calls sequentially (`await` one, then the next, then the next), even though none of those calls depended on each other's results — meaning total wait time was the *sum* of every call, not the slowest one. All converted to fire concurrently via `Promise.allSettled()` (chosen over `Promise.all()` so one endpoint failing — e.g. Inventory being briefly unreachable — doesn't cancel the others, matching each call's existing individual error handling). **Measured, not assumed:** timed the 6 calls behind a typical page load individually (17-135ms each, ~320ms summed) — run in parallel, total wait becomes roughly the single slowest call (~135ms) instead of the sum of all six. The gap will be more noticeable on a slower connection or when Inventory itself is a little slow to respond, since that latency no longer blocks every other independent request queued behind it.
- **Cart-aware stock display (client-side only)** — the product grid's "X in stock" badge previously always showed the raw server number, so a cashier could add 3 Cokes to their cart and the grid would still say "3 in stock" as if none had been taken. Added `reservedBaseQuantity()`: sums, across every cart line for a product (any unit — a Box and loose Pieces of the same product both count), how much is already reserved in the *cashier's own cart*, and the grid now displays `stock_quantity - reserved`, live, re-rendering on every cart change (add/remove/quantity edit). This is a pure UI adjustment — `product.stock_quantity` itself, and the real Inventory record, are never touched by this; the actual deduction still only happens server-side at successful checkout, exactly as before. As a side effect this also closed a real (if narrow) gap in `maxSellableQuantity()`: adding a second *unit* of the same product (e.g. a Box after already adding loose Pieces) previously only checked the raw stock number, not what the first line had already reserved — could let a cashier queue up more of a product across two cart lines than physically exists. Now `addToCart()` subtracts what every other line for that product already reserves before computing how many more of the new unit can be added.
